...
static void nautilus_terminal_instance_init (NautilusOpenTerminal      *cvs);
static void nautilus_terminal_class_init    (NautilusOpenTerminalClass *class);
...
desktop_opens_home_dir (void)
{
	return gconf_client_get_bool (gconf_client,
				      "/apps/nautilus-terminal/desktop_opens_home_dir",
				      NULL);
}
...
{
	return gconf_client_get_bool (gconf_client,
				      "/apps/nautilus-terminal/display_mc_item",
				      NULL);
}
...
static void
terminal_on_screen (const char *command,
			 GdkScreen  *screen)
{
	GAppInfo *app;
...
static void
terminal (NautilusMenuItem *item,
	       NautilusFileInfo *file_info)
{
	...
	if (terminal_command != NULL) {
		terminal_on_screen (terminal_command, screen);
	}
	g_free (terminal_command);
}

static void
terminal_callback (NautilusMenuItem *item,
			NautilusFileInfo *file_info)
{
	terminal (item, file_info);
}

static NautilusMenuItem *
terminal_menu_item_new (NautilusFileInfo *file_info,
			     ...
			     gboolean          is_file_item)
{
	...
	if (command_to_run != NULL) {
		action_name = g_strdup_printf (remote_terminal ?
					       "NautilusOpenTerminal::remote_terminal_%s" :
					       "NautilusOpenTerminal::terminal_%s",
					       command_to_run);
	} else {
		action_name = g_strdup (remote_terminal ? 
					"NautilusOpenTerminal::remote_terminal" :
					"NautilusOpenTerminal::terminal");
	}
	ret = nautilus_menu_item_new (action_name, name, tooltip, "terminal");
	g_free (action_name);
...
static GList *
nautilus_terminal_get_background_items (NautilusMenuProvider *provider,
					     GtkWidget		  *window,
					     NautilusFileInfo	  *file_info)
{
	gchar *uri;
	...
	if (terminal_file_info == FILE_INFO_SFTP ||
		...
		item = terminal_menu_item_new (file_info, terminal_file_info, gtk_widget_get_screen (window),
	...
	if (display_mc_item () &&
	    ...
	    ((terminal_file_info == FILE_INFO_DESKTOP &&
	     ...
	     uri_has_local_path (uri))) {
		item = terminal_menu_item_new (file_info, terminal_file_info, gtk_widget_get_screen (window), "mc", FALSE, FALSE);
		items = g_list_append (items, item);
	}
...
GList *
nautilus_terminal_get_file_items (NautilusMenuProvider *provider,
				       GtkWidget            *window,
				       GList                *files)
{
	gchar *uri;
	...
	switch (terminal_file_info) {
		...
		case FILE_INFO_OTHER:
			if (terminal_file_info == FILE_INFO_SFTP || uri_has_local_path (uri)) {
				item = terminal_menu_item_new (files->data, terminal_file_info, gtk_widget_get_screen (window),
			...
			if (terminal_file_info == FILE_INFO_SFTP &&
			    uri_has_local_path (uri)) {
				item = terminal_menu_item_new (files->data, terminal_file_info, gtk_widget_get_screen (window), NULL, FALSE, TRUE);
				items = g_list_append (items, item);
			}
			...
			if (display_mc_item () &&
			    g_find_program_in_path ("mc") &&
			     uri_has_local_path (uri)) {
				item = terminal_menu_item_new (files->data, terminal_file_info, gtk_widget_get_screen (window), "mc", TRUE, FALSE);
				items = g_list_append (items, item);
			}
			break;
...
static void
nautilus_terminal_menu_provider_iface_init (NautilusMenuProviderIface *iface)
{
	iface->get_background_items = nautilus_terminal_get_background_items;
	iface->get_file_items = nautilus_terminal_get_file_items;
}

static void 
nautilus_terminal_instance_init (NautilusOpenTerminal *cvs)
{
}

static void
nautilus_terminal_class_init (NautilusOpenTerminalClass *class)
{
...
static void
nautilus_terminal_class_finalize (NautilusOpenTerminalClass *class)
{
	g_assert (gconf_client != NULL);
...
GType
nautilus_terminal_get_type (void) 
{
	return terminal_type;
}

void
nautilus_terminal_register_type (GTypeModule *module)
{
	static const GTypeInfo info = {
		sizeof (NautilusOpenTerminalClass),
		(GBaseInitFunc) NULL,
		(GBaseFinalizeFunc) NULL,
		(GClassInitFunc) nautilus_terminal_class_init,
		(GClassFinalizeFunc) nautilus_terminal_class_finalize,
		NULL,
		sizeof (NautilusOpenTerminal),
		0,
		(GInstanceInitFunc) nautilus_terminal_instance_init,
	};

	static const GInterfaceInfo menu_provider_iface_info = {
		(GInterfaceInitFunc) nautilus_terminal_menu_provider_iface_init,
		NULL,
		NULL
	};
